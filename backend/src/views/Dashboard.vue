    ...
    <div class="grid grid-rows-1 md:grid-rows-2 md:grid-flow-col grid-cols-1 md:grid-cols-3 gap-3">
        <div class="col-span-1 md:col-span-2 row-span-1 md:row-span-2 bg-white py-6 px-5 rounded-lg shadow">
            ...
            <template v-if="!loading.latestOrders">
                <div v-for="o of latestOrders" :key="o.id" class="py-2 px-3 bg-blue-100 rounded hover:bg-gray-50">
                    <p>
                        <router-link :to="{name: 'app.orders.view', params: {id: o.id}}" class="text-indigo-700 font-semibold">
                            Order #{{ o.id }}
                        </router-link>
                        created {{ o.created_at }}. Ordered {{ o.items.length }} items
                    </p>

                    <template v-for="(item, index) of o.items" :key="item.id">
                        <div class="flex items-center justify-between py-2 hover:bg-gray-200">
                            <p> ({{ index+1 }}): Product Title: {{ item.product.title.slice(0, 30) }}
                            </p>
                            <p>Quantity: {{ item.quantity }}</p>
                        </div>
                    </template>
                    <p class="flex justify-between">
                        <span>Name: {{ o.customer.first_name }} {{ o.customer.last_name }}</span>
